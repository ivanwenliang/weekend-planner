...
def scrapeEventList(url,database):
	...
	for container in containers:
		...
		event_info_dict['event_name'] = pullTextFromTag(container, 'div', event_tag_classes['event_name'])

		#pull event month and day and concatenate them
		event_month = pullTextFromTag(container, 'p', event_tag_classes['event_month'])
		event_day = pullTextFromTag(container, 'p', event_tag_classes['event_day'])
		if event_month != None and event_day != None:
		    event_info_dict["event_date"] = str(event_month + " " + event_day)

		event_info_dict["event_location"] = pullTextFromTag(container, 'div', event_tag_classes['event_location'])
		...
		event_info_dict['event_desc'] = scrapeEventDesc(container) 
...
def pullTextFromTag(container,html_tag,html_class):
